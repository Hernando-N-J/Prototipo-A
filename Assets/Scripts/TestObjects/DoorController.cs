...
public class DoorController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (enterBool)
            transform.localPosition = Vector3.Lerp(transform.localPosition, Vector3.up * 2, moveSp);

        if (exitBool)
            transform.localPosition = Vector3.Lerp(transform.localPosition, Vector3.zero, moveSp);
    }
...
// if (enterBool) transform.position = Vector3.Lerp(startPos, Vector3.up * 2, a);
